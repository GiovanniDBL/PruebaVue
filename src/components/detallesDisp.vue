            ...
            <div class="tab-pane fade show active" id="pills-graficas" role="tabpanel" aria-labelledby="pills-graficas-tab" tabindex="0">
                ...
                <div class="row" style="margin-bottom:1rem">
                    ...
                    <div class="col-4">
                        ...
                        <div class="card ">
                            <div class="card-header">
                                Último indicador de temperatura
                                <div style="float:right" class="dropdown">
                                    <i style="font-size: 21px;" type="button" class="far fa-ellipsis-v" id="OptionsTemp" data-bs-toggle="dropdown" aria-expanded="false"></i>
                                    <ul class="dropdown-menu animated fadeIn fast" aria-labelledby="OptionsTemp">
                                        <li><a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#Reglas" href="#">Reglas</a></li>
                    ...
                    <div class="row">
                        ...
                        <div class="col-5">
                            ...
                            <div class="card" style="margin-top:1rem">
                                <div class="card-header">
                                    <i style="color:#1abb97; font-size:18px" class="fas fa-info-circle"></i>
                                    Historial Aperturas/Cierres
                                    <!-- <span style="float:right"><i class="fas fa-clock"></i> Últimas 24 horas</span> -->

                                    <div style="float:right;margin-left:5px">

                                        <select v-on:change="OnchangeAperturas($event)" v-model="SelectApertura" class="form-select form-select-sm" aria-label=".form-select-sm example">
                                            <!-- <option selected></option> -->
                                            <option value="24 hours" selected>Últimas 24 horas</option>
                                            <option value="7 days">Últimos 7 días</option>
                                            <option value="15 days">Últimos 15 días</option>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body ">
                                    <div class="scrollable-alarmas">
                                        <table id="Talarmas" cellspacing="1" cellpadding="1" class="table table-tamaño table-hover">
                                            <thead class="">
                                                <tr class="t-head-table">
                                                    <th scope="col">ID</th>
                        ...
                        <div class="col-6">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <span class="position-relative">
                                        Últimos Eventos
                                        <span style="margin-left:1rem" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"> {{totalEvents}}</span>
                                    </span>
                                    <div style="float:right" class="btn-group " role="group" aria-label="Basic mixed styles example">
                                        <button @click="PDF_Teventos" type="button" class="btn btn-success btn-sm">PDF <i type="button" class="fas fa-download"></i></button>
                                    </div>
                                </div>
                                <div class="card-body ">
                                    <div class="scrollable-alarmas">
                                        <table id="Teventos" cellspacing="1" cellpadding="1" class="table table-tamaño table-hover">
                                            <thead class="">
                                                <tr class="t-head-table">
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Nombre</th>
 ...
 destroyed() {
    ...
    methods: {
        ...
        PDF_Talarmas() {
            ...
            pdf.text('Alarmas del dispositivo ' + this.NombAsignadoForm, width / 2, 40, {
                align: 'center'
            });
            pdf.autoTable({
                html: '#Talarmas',
        ...
        PDF_Teventos() {
            ...
            pdf.text('Eventos del dispositivo ' + this.NombAsignadoForm, width / 2, 40, {
                align: 'center'
            });
            pdf.autoTable({
                html: '#Teventos',
        ...
        dowloadPdf() {
            ...
            pdf.text(20, 50, 'ID_dispositivo:');
            pdf.text(20, 60, 'Nombre:');
            pdf.text(20, 70, 'Subcuenta:');
            // pdf.text(20, 70, 'Fecha de creación:');
            pdf.text(20, 80, 'Ubicación interna:');
            pdf.text(20, 90, 'Ciudad:');
            pdf.text(20, 100, 'Dirección:');
            pdf.text(20, 110, 'Última Temperatura:');
            pdf.text(20, 120, 'Última Vibración:');
            pdf.text(20, 130, 'Última Gas:');
            // pdf.text(20, 140, 'Última Voltaje:');

            pdf.setTextColor('#4472C4')
            pdf.text(62, 50, this.NameDeviceForm);
            pdf.text(45, 60, this.NombAsignadoForm);
            pdf.text(53, 70, this.SucursalForm);
            // pdf.text(65, 70, '19/11/2021 11:12:56');
            pdf.text(69, 80, this.UbicSensorForm);
            pdf.text(41, 90, this.CiudadForm);
            pdf.text(49, 100, this.UbicacionForm);
            pdf.text(75, 110, this.ultimatemp);
            pdf.text(70, 120, this.ultimavib);
            pdf.text(55, 130, this.ultimagas);
            // pdf.text(65, 140, '0');
            ...
            xhr.onload = () => {
                ...
                if (resp.data !== undefined) {
                    ...
                    if (this.center.lat == 0 & this.center.lng == 0) {
                        this.center.lat = 23.634501;
                        this.center.lng = -102.552784;
                    }
                    ...
                    if (this.markers[0].position.lat == 0 & this.markers[0].position.lng == 0) {
                        this.markers[0].position.lat = 23.634501;
                        this.markers[0].position.lng = -102.552784;
                    }
        ...
        getAperturas(data) {
            ...
            xhr.onload = () => {
                ...
                this.spinner = false;
                // this.totalEvents = json.length;
                if (resp[0].result == 'No se encontraron resultados') {
                    this.mensaje = true
                }
                for (var index in json) {
        ...
        OnchangeAperturas(event) {
            // this.spinner = '';
            this.AperturasDisp.length = 0;
            this.mensaje = '';
            this.SelectApertura = event.target.value;
            var timeUtc = this.time.utc().format();
            this.dataAperturaSelect = {
                "typeFunction": "getDatos_apertura_device",
                "idDevice": this.idDeviceSelected,
  ...
  switch (this.SelectGraficas) {
        ...
        WebSocketTest(ws) {
            ...
            ws.onmessage = (evt) => {
                ...
                console.log("Mensaje Recibido...", this.idDeviceSelected);
